...
const EditChat = () => {
  ...
  const fetchChatData = async () => {
    try {
      let { data : {data} } = await axios.get(`/admin/chat/${id}`)  
  ...
  const onSubmit = async (formData) => {
    try {
      let { data } = await axios.post(`/admin/update-chat/${id}`,{
    ...
    <form  onSubmit={handleSubmit(onSubmit)} className="w-full">
        ...
        <div className="flex flex-wrap -mx-3 mb-6">
          <div className="w-full px-3 mb-6 md:mb-0">
            ...
            <textarea {...register("prompt")} className="appearance-none block w-full bg-gray-200 text-gray-700 border  rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="grid-first-name" type="text" placeholder="Add Prompt . . . " ></textarea>
