...
function ChangePassword(props) {
  const { userData , updateLang ,allChats ,updateUserChats} = props
  ...
  let userChats = userData.chats.map((chat)=>{
    return {
      label:chat.name,
      value:chat.id
    }
      ...
      <div className="fixed inset-0 z-10 w-screen overflow-y-auto">
        <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
          ...
          {/* <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
            ...
            <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
              <button onClick={()=>updateUserChats(selected)} type="button" className="inline-flex w-full justify-center rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-500 sm:ml-3 sm:w-auto">Update User Chats</button>
              <button onClick={()=>{props.setModal(false)}} type="button" className="mt-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto">Cancel</button>
            </div>
          </div>
        </div>
